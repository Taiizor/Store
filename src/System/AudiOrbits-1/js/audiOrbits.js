...
var audiOrbits = {
	...
	applyCustomProps: function(props) {
		...
		if (self.debug && !self.debugTimeout)
			self.debugTimeout = setTimeout(() => self.applyCustomProps({
				debugging: {
					value: false
				}
			}), 1000 * 60);
...
function SucrosePropertyListener(name, val) {
	...
	switch (name) {
		case "parallax_option":
			sett.parallax_option = val.value;
			break;
		case "parallax_strength":
			sett.parallax_strength = val.value;
			break;
		case "fps_30":
			if (val.value)
				sett.fps_value = 30;
		...
		case "default_brightness":
			sett.default_brightness = val.value;
			break;
		case "default_saturation":
			sett.default_saturation = val.value;
			break;
		case "zoom_val":
			sett.zoom_val = val.value;
			break;
		case "rotation_val":
			sett.rotation_val = val.value;
			break;
		case "main_color":
			document.body.style.backgroundColor = "#" + val.value.substring(3);
			break;
		case "color_mode":
			sett.color_mode = val.value;
			self.initHueValues();
			break;
		case "user_color_a":
			tmp = hexToRgb("#" + val.value.substring(3));
...
function argbToRgb(argb) {
	...
	var blue = argb & 0xFF;
	var rgbHex = (red << 16) | (green << 8) | blue;
