...
function SucroseAudioData(audioArray) {
	maxVal = 1;
	for (var x of audioArray.Data) {
		if (x > maxVal) maxVal = x;
	}

	const offSet = vizWidth / audioArray.Data.length;
	const arrMid = audioArray.Data.length / 2;
	...
	let posInLine = -1;
	for (var x = 0; x < audioArray.Data.length; x++) {
		posInLine++;
		ctx.lineTo(
			startPos + offSet * posInLine,
			midY - (audioArray.Data[x] / maxVal) * max_height
		);
		if (square)
			ctx.lineTo(
